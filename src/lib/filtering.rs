...
pub fn filter_by_n(input_path: &String, ambiguity: &f32) -> io::Result<()> {
    ...
    for record in fasta_reader.records() {
        ...
        if count_n >= tolerance {
            println!("Record {} had more than {} masked bases and is thus removed.", id, tolerance)
        } else {
            fasta_writer.write(id,record.desc(), sequence).expect("Error writing record");
        }
    }
...
pub fn date_accessions(input_path: &str) -> Result<HashMap<String, String>, PolarsError> {
    ...
    if !df.schema().contains("Accession") || !df.schema().contains("Isolate Collection date") {
        return Err(PolarsError::ColumnNotFound("Required columns not found".into()));
    }

    // Create a HashMap from the "Accession" and "Isolate Collection date" columns
    let accession_s = df.column("Accession")?.cast(&DataType::Utf8).expect("Cannot cast to Utf8");
    let date_s = df.column("Isolate Collection date")?.cast(&DataType::Utf8).expect("Cannot cast to Utf8");
    
    let mut accession_to_date = HashMap::new();
    for (accession, date) in accession_s.utf8().unwrap().into_iter().zip(date_s.utf8().unwrap().into_iter()) {
...
pub fn separate_by_month(input_fasta: &str, accession_to_date: HashMap<String, String>) -> io::Result<()> {
    ...
    for record in reader.records() {
        ...
        match record_date {
            ...
            None => println!("Skipping record with missing or unparseable date: {}", record.id()),
        }
    }
